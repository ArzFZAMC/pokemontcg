...
export default function Battle() {
  ...
  const setupSocket = useCallback(() => {
    ...
    socket.on('battle_phase_change', ({ phase: p }) => {
      console.log('Phase change:', p);
      if (p === 'selecting') updatePhase(BATTLE_PHASES.SELECTING);
  ...
  const handleSubmitCards = async (selectedCards) => {
    ...
    try {
      const res = await API.post('/battle/cards/submit', { room_id: roomId, cards: selectedCards });
      setIReady(true);
  ...
  const handleAttack = (attackIndex) => { setAttackLoading(true); socketRef.current?.emit('attack', { roomId, attackIndex }); };
  ...
  return (
    <div className="space-y-5 page-transition max-w-2xl mx-auto">
                ...
                {battleStats.recent.slice(0, 5).map((b, i) => {
                  ...
                  return (
                    <div key={i} className="flex items-center justify-between text-xs p-2 rounded-lg bg-navy-700/50">
                      <span className={won ? 'text-green-400 font-semibold' : 'text-red-400'}>{won ? '🏆 Won' : '💀 Lost'}</span>
                      <span className="text-white/40">{won ? b.winner_name : b.loser_name} vs {won ? b.loser_name : b.winner_name}</span>
      ...
      {phase === BATTLE_PHASES.BATTLE && roomData && (
        <BattleArena room={roomData} cards={cards} logs={logs} myId={user.id} isMyTurn={isMyTurn}
